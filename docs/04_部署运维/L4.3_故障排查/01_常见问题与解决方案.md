---
title: OpenTelemetry 故障排查指南
description: OpenTelemetry 故障排查指南 详细指南和最佳实践
...
**本模块思维表征**：故障排查的**决策树**与问题分类见 [📊 多维思维表征体系 §10 按文档模块的思维表征索引](../../📊_多维思维表征体系_2025.md#10-按文档模块的思维表征索引)（08_故障排查），以及 [00_问题诊断决策树手册](../00_问题诊断决策树手册.md)。
...
flowchart LR
  A[现象] --> B{问题类型?}
  B -->|连接| C[检查Collector/网络/TLS]
  B -->|数据丢失| D[采样/队列/导出]
  B -->|性能| E[延迟/CPU/内存]
  B -->|数据质量| F[属性/关系/时间戳]
  B -->|Collector| G[崩溃/队列/导出]
  B -->|K8s| H[Pod/Resource/ServiceMesh]
  B -->|后端| I[Jaeger/Prometheus/存储]
```
