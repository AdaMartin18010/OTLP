---
title: OTLP协议概述
description: OTLP协议概述 详细指南和最佳实践
...
**本模块思维表征**：OTLP 核心协议的**思维导图**、**多维矩阵**（gRPC vs HTTP、Protobuf vs JSON）、**定理/公理推理树**与**决策树**（传输选型）见 [📊 多维思维表征体系 §10 按文档模块的思维表征索引](../../📊_多维思维表征体系_2025.md#10-按文档模块的思维表征索引)（01_OTLP核心协议行）。
...
- [OTLP协议概述](#otlp协议概述)
  ...
  - [1. 概念定义](#1-概念定义)
    ...
    - [1.2 通俗解释](#12-通俗解释)
    - [1.3 标准引用](#13-标准引用)
    - [1.4 本项目内容范围（三条范围及文档入口）](#14-本项目内容范围三条范围及文档入口)
  - [2. 核心特性](#2-核心特性)
    - [2.1 信号支持](#21-信号支持)
    - [2.2 传输协议](#22-传输协议)
...
### 1.4 本项目内容范围（三条范围及文档入口）

本仓库文档围绕以下三条内容范围组织，便于与权威规范对齐与检索：

| 范围 | 定义要点 | 应用场景 | 本项目文档入口 |
|------|----------|----------|----------------|
| **OpenTelemetry OTLP** | trace/metric/log 的传输、序列化（gRPC/HTTP）、语义约定 | 可观测性、微服务遥测 | [01_OTLP核心协议](../01_OTLP核心协议/)（本目录）、[02_Semantic_Conventions](../02_Semantic_Conventions/) |
| **OTLP Metrics Data Model** | 支持 pre-aggregated 时序数据、与 Prometheus/StatsD 双向映射 | 监控、指标收集 | [03_数据模型/02_Metrics数据模型](../03_数据模型/02_Metrics数据模型/)、[03_Pre-aggregation与Prometheus_StatsD映射](../03_数据模型/02_Metrics数据模型/03_Pre-aggregation与Prometheus_StatsD映射.md) |
| **OTLP Logs Data Model** | 统一日志结构（Event、属性、资源）、多种日志格式无歧义映射 | 日志聚合、审计 | [03_数据模型/03_Logs数据模型](../03_数据模型/03_Logs数据模型/)、[03_统一结构与多格式映射](../03_数据模型/03_Logs数据模型/03_统一结构与多格式映射.md) |

范围与权威来源的对齐、缺口及思维表征索引见 [00_范围-权威对齐矩阵](../🔬_批判性评价与持续改进计划/00_范围-权威对齐矩阵.md)。
...
2024-09: Semantic Conventions v1.41.0.0
2025-10: 本文档更新时间点
```
