---
title: OTLP传输层 - gRPC详解
description: OTLP传输层 - gRPC详解 详细指南和最佳实践
...
**OTLP 传输选型决策树**（gRPC vs HTTP，详见 [03_传输层_HTTP](./03_传输层_HTTP.md) 对比）：

```mermaid
flowchart TD
  Start[选择 OTLP 传输] --> Q1{需要流式/双向?}
  Q1 -->|是| gRPC[gRPC 4317]
  Q1 -->|否| Q2{环境限制?}
  Q2 -->|防火墙仅允许 HTTP| HTTP[HTTP 4318]
  Q2 -->|无限制| Q3{性能优先?}
  Q3 -->|是| gRPC
  Q3 -->|否| HTTP
  gRPC --> Doc1[本文档]
  HTTP --> Doc2[03_传输层_HTTP]
```

**gRPC vs HTTP 多维对比矩阵**：

| 维度 | gRPC (本文档) | HTTP ([03_传输层_HTTP](./03_传输层_HTTP.md)) |
|------|----------------|---------------------------------------------|
| 默认端口 | 4317 | 4318 |
| 底层协议 | HTTP/2 | HTTP/1.1 |
| 编码 | Protobuf 二进制 | Protobuf 或 JSON |
| 适用场景 | 高性能、服务端推送、流式 | 代理友好、浏览器、简单调试 |
| 连接 | 长连接、多路复用 | 请求-响应、无状态 |
