...
**本模块思维表征**：采样与性能的**决策树**（采样策略选择、性能调优路径）见 [📊 多维思维表征体系 §10.3 采样与性能、实战案例、安全与合规](../../📊_多维思维表征体系_2025.md#103-采样与性能实战案例安全与合规)。
...
flowchart TD
  ...
  T -->|是| H[Head-based]
  T -->|否| L[Tail-based]
  H --> H1[TraceIDRatio/固定比例]
  H --> H2[ParentBased]
  H --> H3[RateLimiting]
  L --> L1[Collector TailSampling]
  L --> L2[按错误/延迟等策略]
```
