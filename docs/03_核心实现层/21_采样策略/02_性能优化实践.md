---
title: OpenTelemetry 性能优化实践指南
description: OpenTelemetry 性能优化实践指南 详细指南和最佳实践
...
| 层级 | 优化方向 | 典型手段 |
|------|----------|----------|
| SDK | 批处理、异步、采样、属性限制 | BatchSpanProcessor、采样率、属性截断 |
| Collector | 资源配置、Pipeline、内存 | 批处理、内存限制 |
| 网络 | 压缩、连接复用、协议 | gzip、Keep-Alive、gRPC |
| 数据量 | 采样、基数控制 | 头/尾采样、标签裁剪 |
...
flowchart TD
  P[性能问题] --> Q{瓶颈?}
  Q -->|SDK 开销| SDK[§2 SDK 优化]
  Q -->|Collector| Coll[§3 Collector 优化]
  Q -->|网络/存储| Net[§4–§6 网络与存储]
```
